...
save_picks()  -- write generated picks to the picks table (deduped per week).
...
a couple of weeks of results is noise; judge over 100+ graded picks.
...
def save_picks(picks: list[dict], game_date: str | None = None) -> int:
    """Insert this week's picks, skipping ones already saved for this date."""
    if not picks:
        return 0
...
def _fetch_results(lookback: int = 10) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not player_df.empty:
        for col in ("rushing_tds", "receiving_tds", "rushing_yards", "receiving_yards", "passing_yards"):
            if col not in player_df.columns:
                player_df[col] = 0.0
        player_df["rush_rec_tds"] = player_df["rushing_tds"].fillna(0) + player_df["receiving_tds"].fillna(0)
        player_df["rush_rec_yards"] = player_df["rushing_yards"].fillna(0) + player_df["receiving_yards"].fillna(0)
        player_df["pass_rush_yards"] = player_df["passing_yards"].fillna(0) + player_df["rushing_yards"].fillna(0)
        player_df["total_yards"] = (player_df["passing_yards"].fillna(0) + player_df["rushing_yards"].fillna(0)
                                     + player_df["receiving_yards"].fillna(0))
        player_df["fantasy"] = (
            player_df["passing_yards"].fillna(0) * 0.04 + player_df.get("passing_tds", 0).fillna(0) * 4
            - player_df.get("interceptions", 0).fillna(0) * 2
            + player_df["rushing_yards"].fillna(0) * 0.1 + player_df["rushing_tds"].fillna(0) * 6
            + player_df["receiving_yards"].fillna(0) * 0.1 + player_df["receiving_tds"].fillna(0) * 6
            + player_df.get("receptions", 0).fillna(0) * 1.0
        )
    return team_df, player_df


def _close_date(d1: str, d2: str) -> bool:
    """ESPN dates are UTC, pick dates are Central-anchored — evening games can
    differ by one calendar day."""
    try:
