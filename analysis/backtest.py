...
this is the number to trust when judging a model change, not two weeks of picks.
...
def run_backtest(test_season: int | None = None) -> dict | None:
    ...
    conn = get_conn()
    matchups = build_matchup_features(build_team_rolling_features(logs), conn)
    conn.close()
    ...
    train_df = matchups[matchups["season_int"] < test_season].sort_values("date")
    test_df = matchups[matchups["season_int"] == test_season].sort_values("date")
    if test_df.empty:
    ...
    print("-" * 60)
    print(f"  Moneyline accuracy:  {acc:.3f}   (home team wins ~55-57% as baseline)")
