"""
Fetches live NFL player prop lines from PrizePicks.
...
Two code paths deliberately diverge here (see CLAUDE.md's two-entry-points split):
  ...
  * fetch_nfl_lines()      — plain requests. Used by the CLOUD app (ui/app_cloud.py). It
...
NFL's league_id isn't hardcoded from memory — resolve_nfl_league_id() looks it
up from PrizePicks' /leagues endpoint and caches the result, falling back to a
best-effort default if that lookup itself gets DataDome-blocked. Run the
__main__ smoke block once after first deploy to confirm the resolved id and
that props are actually flowing (stat-type counts printed below).
"""
import sys, os
...
from pipeline.team_names import TEAM_ABBR_TO_NAME
...
_LEAGUE_ID_PATH = os.path.join(os.path.dirname(__file__), "..", "data", ".pp_nfl_league_id.json")

# Commonly-published PrizePicks NFL league_id — unverified against a live call.
# resolve_nfl_league_id() confirms this against /leagues at runtime.
_DEFAULT_NFL_LEAGUE_ID = 9
...
def _load_cached_league_id() -> int | None:
    try:
        with open(_LEAGUE_ID_PATH) as f:
            return json.load(f).get("league_id")
    except Exception:
        return None


def _save_cached_league_id(league_id: int) -> None:
    try:
        os.makedirs(os.path.dirname(_LEAGUE_ID_PATH), exist_ok=True)
        with open(_LEAGUE_ID_PATH, "w") as f:
            json.dump({"league_id": league_id}, f)
    except Exception:
        pass


def resolve_nfl_league_id() -> int:
    """Look up PrizePicks' current NFL league_id from /leagues, caching the
    result locally. Falls back to _DEFAULT_NFL_LEAGUE_ID if the lookup itself
    fails (e.g. the same DataDome protection that blocks fetch_nfl_lines on cloud)."""
    ...
    try:
        ...
        for item in resp.json().get("data", []):
            ...
            if attrs.get("name") == "NFL" or attrs.get("sport") == "NFL":
                league_id = int(item["id"])
                _save_cached_league_id(league_id)
                return league_id
    except Exception as e:
        print(f"  Warning: could not resolve NFL league_id from /leagues ({e}); "
              f"using default {_DEFAULT_NFL_LEAGUE_ID}")
    return _DEFAULT_NFL_LEAGUE_ID


def _params(league_id: int) -> dict:
    return {"league_id": league_id, "per_page": 250, "single_stat": "true"}


def _proj_url(league_id: int) -> str:
    return f"{PRIZEPICKS_URL}?league_id={league_id}&per_page=250&single_stat=true"
...
def fetch_nfl_lines() -> list[dict]:
    ...
    league_id = resolve_nfl_league_id()
    resp = requests.get(PRIZEPICKS_URL, headers=HEADERS, params=_params(league_id), timeout=15)
    resp.raise_for_status()
...
def _parse_projections(data: dict) -> list[dict]:
    ...
    for proj in projections:
        ...
        allowed = "More" if odds_type in ("demon", "goblin") else None

        raw_abbr = pinfo.get("team", "")
        player_team = TEAM_ABBR_TO_NAME.get(raw_abbr.upper(), raw_abbr)
...
def _fetch_with_cookie(datadome: str, user_agent: str):
    ...
    return creq.get(
        PRIZEPICKS_URL,
        params=_params(resolve_nfl_league_id()),
...
def get_prizepicks_lines() -> list[dict]:
    ...
    fetch_nfl_lines() instead and is unaffected by this path."""
...
if __name__ == "__main__":
    print(f"Resolved NFL league_id: {resolve_nfl_league_id()}")
    props = get_prizepicks_lines()
    print(f"Fetched {len(props)} PrizePicks NFL props:")
    stats = {}
    for p in props:
        stats[p["stat_type"]] = stats.get(p["stat_type"], 0) + 1
    for stat, cnt in sorted(stats.items(), key=lambda x: -x[1])[:15]:
        print(f"  {stat}: {cnt}")
