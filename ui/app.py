"""
NFL Bet -- Streamlit Dashboard (local, uses SQLite)
Tabs: Top Picks | Game Predictions | Player Props | Platform Comparison | Bankroll | Ask Why
"""
import sys, os, time
...
from pipeline.schedule import get_week_games, load_saved_games
...
with st.sidebar:
    ...
    if st.button("Refresh All Data", use_container_width=True):
        # Each refresh costs Odds API credits (3 markets); block accidental
        # double-clicks instead of silently re-spending them.
        ...
        if time.time() - last < 60:
            ...
            with st.spinner("Refreshing..."):
                get_week_games()
                # PrizePicks is behind DataDome bot protection and can 403;
...
def load_data(bankroll, unit_size):
    # Read the saved schedule -- get_week_games() burns an Odds API credit per
    ...
    games = load_saved_games() or get_week_games()
...
for _g in games:
    try:
        ...
        _date = _ct.strftime("%a %b %d")
        _time = _ct.strftime("%-I:%M %p")
    except Exception:
        _date = _g.get("date", "--")
        _time = "--"
    _h, _a = _g["home_team"], _g["away_team"]
    # "sort" = ISO UTC kickoff, used to order tables chronologically -- the
    # display strings ("Sun Sep 14", "3:25 PM") don't sort correctly as text.
...
def timestamp_bar():
    ...
    # NFL is weekly, not daily -- a 6h staleness threshold (wnba-bet's) would
    # flag every Tuesday-Wednesday as "stale" even right after a Monday refresh.
    stale = worst > 48
    border = "#dc2626" if stale else "#22c55e"
    warn = ("  &nbsp;&middot;&nbsp; <strong style='color:#f87171'>stale -- hit Refresh in the sidebar</strong>"
            if stale else "")
    st.markdown(
        ...
        f"Data updated -- {' &nbsp;&middot;&nbsp; '.join(parts)}{warn}</div>",
    ...
    for p in picks:
        if p["pick_type"] == "game":
            ...
            row = {
                ...
                "Kickoff":   f"{gi.get('date', '--')} {gi.get('time', '--')}",
            ...
            if ot in ("goblin", "demon"):
                sel += {"goblin": " (goblin)", "demon": " (demon)"}[ot]
            row = {
                "_sort":     gi.get("sort", "~"),
                "Game":      gi.get("opp", "--"),
                "Kickoff":   f"{gi.get('date', '--')} {gi.get('time', '--')}",
...
with tab1:
    ...
    if df is not None:
        ...
        st.info("No games found -- hit Refresh in the sidebar to fetch this week's schedule.")
    elif not all_picks:
        st.info("Games found, but no picks cleared the edge threshold this week.")
    else:
...
with tab2:
    ...
    if not games:
        ...
        for g in games:
            ...
            v = views.get(home)
            qb_flag = " · QB OUT ALERT" if v and (v.get("home_qb_out") or v.get("away_qb_out")) else ""

            with st.expander(f"**{away}** @ **{home}**{tip}{qb_flag}", expanded=len(g_picks) > 0):
                if v and (v.get("home_qb_out") or v.get("away_qb_out")):
...
with tab5:
    ...
    with col1:
        ...
        st.caption("Graded picks from analysis/tracking.py (start.sh grades last week's picks each run).")
        if wl.empty:
