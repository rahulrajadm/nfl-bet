"""
NFL Bet -- Streamlit Community Cloud version.
Fetches all data in-memory (no SQLite). Refresh is passcode-gated.
"""
import sys, os, shutil, pathlib

# Same class of Linux-only segfault wnba-bet hit on Streamlit Cloud (identical
# versions render fine on macOS). faulthandler prints every thread's Python
# stack to stderr on SIGSEGV, turning a bare "Segmentation fault" into an
...
# Arrow's Linux wheels default to jemalloc, macOS wheels to mimalloc, which is
# why a jemalloc-related segfault (seen on wnba-bet, 2026-07-10) never
# reproduces locally on identical versions. ARROW_DEFAULT_MEMORY_POOL is read
...
from pipeline.prizepicks import fetch_nfl_lines as pp_fetch
from pipeline.underdog import fetch_nfl_lines as ud_fetch
...
st.set_page_config(page_title="NFL Bet", page_icon="\U0001F3C8", layout="wide", initial_sidebar_state="expanded")
...
def fetch_odds_and_schedule() -> tuple[list[dict], list[dict]]:
    ...
    try:
        ...
        for game in resp.json():
            ...
            # Store date in Central so evening/night games (kickoff ~7-8pm CT)
            # aren't mis-labelled as the next calendar day.
            game_date = game_dt.astimezone(ZoneInfo("America/Chicago")).strftime("%Y-%m-%d")
    ...
    # Odds API often has nothing posted for NFL until close to kickoff week --
    # cap to the next 10 days so the schedule doesn't silently include next
    # month's slate (which would confuse the "this week" framing everywhere else).
    games = [g for g in seen_games.values()
             if date.fromisoformat(g["date"]) <= date.today() + timedelta(days=10)]
    return rows, games
...
def load_all_data():
    ...
    if not games:
        # Odds API has no NFL markets posted yet (common outside the season) --
        # fall back to ESPN so the app still shows something (and props/game
        # model can still be exercised even with zero live odds).
        from pipeline.schedule import fetch_week_games_from_espn
        games = fetch_week_games_from_espn()
    ...
    with ThreadPoolExecutor(max_workers=4) as ex:
        ...
        try:
            ...
            ud_lines = []
        try:
            team_logs = f_team_logs.result(timeout=90)
            if _cloud_data._last_fetch_failures:
                st.warning(f"nflverse: {_cloud_data._last_fetch_failures} season pull(s) failed -- team stats may be incomplete.")
        except Exception:
            team_logs = pd.DataFrame()
        try:
            player_logs = f_player_logs.result(timeout=90)
            if _cloud_data._last_fetch_failures:
                st.warning(f"nflverse: {_cloud_data._last_fetch_failures} season pull(s) failed -- prop stats may be incomplete.")
        except Exception:
            player_logs = pd.DataFrame()

    player_logs = derive_prop_columns(player_logs)
...
with st.sidebar:
    st.title("\U0001F3C8 NFL Bet")
    st.caption("AI-powered NFL betting decisions")
    st.divider()
        ...
        if st.button("Refresh All Data", use_container_width=True):
            ...
            if code == correct and correct != "":
                ...
                if fresh["games"]:
                    ...
                    st.warning("No upcoming games found. Keeping existing data. Try again later.")
            else:
...
def timestamp_bar(fetched_at: str):
    st.markdown(
        ...
        f" &nbsp;&middot;&nbsp; Refresh in the sidebar to update</div>",
    ...
    for p in picks:
        if p["pick_type"] == "game":
            ...
            row = {
                ...
                "Kickoff":   f"{_gi.get('date', '--')} {_gi.get('time', '--')}",
            ...
            if _ot in ("goblin", "demon"):
                sel += {"goblin": " (goblin)", "demon": " (demon)"}[_ot]
            row = {
                "_sort":     _gi.get("sort", "~"),
                "Game":      _gi.get("opp", "--"),
                "Kickoff":   f"{_gi.get('date', '--')} {_gi.get('time', '--')}",
...
with tab1:
    st.caption("Game picks (ML/Spread/Totals) + high-interest player props, ranked by EV. Kickoff times in CT.")
    ...
    if df is not None:
        ...
        st.warning("No upcoming games found. Try refreshing later.")
    elif not all_picks:
...
with tab2:
    ...
    def _model_views(cache_key):
        ...
        for _g in data["games"]:
            _h, _a = _g["home_team"], _g["away_team"]
            try:
                _pred = predict_game(_h, _a, game_date=_g.get("date"),
                                      game_logs_df=data.get("team_logs"),
                                      player_logs_df=data.get("player_logs"),
                                      injury_flags=data.get("injuries"))
            except Exception:
                _pred = None
            if not _pred:
                continue
            ...
            views[_h] = {
                ...
                "home_qb_out": _pred.get("home_qb_out", False), "away_qb_out": _pred.get("away_qb_out", False),
            }
        return views
    ...
    if not games_list:
        ...
        for g in games_list:
            ...
            v = views.get(home)
            qb_flag = bool(v and (v.get("home_qb_out") or v.get("away_qb_out")))
            ...
            if qb_flag:
                ...
                badge += "  ·  injury alert"
            header = f"**{away}** @ **{home}**" + (f"  ·  {tip_label}" if tip_label else "") + badge
            with st.expander(header, expanded=len(g_picks) > 0):
                if qb_flag:
                    flagged = [t for t, out_ in ((home, v.get("home_qb_out")), (away, v.get("away_qb_out"))) if out_]
                    st.warning(f"Starting QB flagged Out/Doubtful+ for: {', '.join(flagged)}")
                if has_injury:
                    parts = []
...
with tab3:
    ...
    if df is not None:
        df = df.drop(columns=["Kickoff", "EV/$100", "Stake", "Win"], errors="ignore")
        st.dataframe(style_df(df), use_container_width=True, hide_index=True)
        st.caption("**Season/Recent** = the player's per-game average for this stat (full season vs last 4).")
    elif not data["games"]:
...
with tab4:
    ...
    for key, plist in grouped.items():
        ...
        if max(lines_vals) - min(lines_vals) > 3.0:
            continue
    ...
    if not items:
        st.info("No cross-platform overlaps at comparable lines this week.")
    else:
        for (player, stat), picks_list in items[:40]:
            picks_list.sort(key=lambda x: x["edge"], reverse=True)
            top = picks_list[0]
            lines_lbl = " vs ".join(f"{p['platform'][:2].upper()} {p['line']}" for p in picks_list)
...
with tab5:
    ...
    if rows:
        ...
        st.info("No picks to stake this week.")
    ...
    with c2:
        st.subheader("Notes")
        st.caption(
            "Stakes are quarter-Kelly per leg sized as a standalone 3x bet -- a real "
...
with tab6:
    st.caption(
        ...
        "Try: *why does Saquon Barkley Rush Yards More 78.5 have 93%?*, then follow up with "
        "*what about a line of 90.5?*, *what about Less?* -- or ask "
        "*safest pass yards pick this week* / *compare Josh Allen across platforms*."
    )
